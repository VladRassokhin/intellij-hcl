...
class TFDuplicatedOutputInspection : TFDuplicatedInspectionBase() {
  ...
  inner class MyEV(val holder: ProblemsHolder) : HCLElementVisitor() {
    override fun visitBlock(block: HCLBlock) {
      if (!TerraformPatterns.OutputRootBlock.accepts(block)) return
      ...
      holder.registerProblem(block, "Output '$name' declared multiple times", ProblemHighlightType.GENERIC_ERROR, *getFixes())
    }
  }
  ...
  private object DeleteOutputFix : LocalQuickFixBase("Delete output") {
    override fun applyFix(project: Project, descriptor: ProblemDescriptor) {
      // TODO Implement
    }
  }
  private object RenameOutputFix : LocalQuickFixBase("Rename output") {
    override fun applyFix(project: Project, descriptor: ProblemDescriptor) {
      // TODO Implement
    }
  }
